---
name: "High-Fidelity Claymorphism Frontend Engineer"
description: "Expert frontend engineer for integrating a High-Fidelity Claymorphism design system focusing on tactile UI, rich colors, and bouncy animations."
---
...
- Ensure the application is fully responsive, with flawless layouts and interactions optimized for both **Mobile devices and PC (Desktop)**.
...
**The Sensory Vibe**:
*   **Playful & Optimistic**: The interface radiates joy through "candy store" colors focusing heavily on **vibrant oceanic and sky blues** as the primary identity, paired with bouncy, organic motion. It feels safe, welcoming, and unpretentious—like a premium toy store display.
...
**Background**:
*   **Canvas**: `#F0F6FA` (Very pale, icy blue-white). This provides a cleaner, more modern base than warm beige. Never use pure white—the slight blue tint creates depth and freshness.

**Foreground**:
*   **Text (Primary)**: `#1E293B` (Slate 800). High contrast (passing WCAG AA) but softer than black for a friendlier feel.
*   **Muted (Secondary)**: `#475569` (Slate 600). Crucial for readability against light backgrounds. Use for body text, labels, and secondary information. Never go lighter than this value.

**Accents (Vibrant & Saturated)**:
*   **Primary Accent**: `#2563EB` (Vivid Blue). The hero color used for primary CTAs, links, and brand emphasis.
*   **Secondary Accent**: `#0EA5E9` (Sky Blue). Used in gradients and for secondary emphasis.
*   **Tertiary**: `#60A5FA` (Light Blue). For informational elements and background blobs.
...
**Gradient Strategy**:
*   **Primary Buttons**: `bg-gradient-to-br from-[#60A5FA] to-[#2563EB]` (lighter blue to primary blue). This creates depth and prevents overly dark buttons.
*   **Icon Orbs**: `bg-gradient-to-br` from light pastel (400) to saturated hue (600) focusing heavily on blues (e.g., `from-blue-300 to-blue-500`, `from-sky-300 to-sky-500`).
*   **Text Highlights**: Use multi-stop gradients for hero text (`clay-text-gradient`): `from-slate-800 20%, to-blue-600 60%, to-sky-500`. Keep gradient text large (text-5xl+) for readability.
*   **Background Blobs**: Semi-transparent blue accent colors with 10% opacity and blur-3xl for soft ambient lighting.
...
```css
box-shadow: 
  30px 30px 60px #cce0f5,           /* Deep, soft ambient occlusion */
  -30px -30px 60px #ffffff,         /* Top-left ambient light */
  inset 10px 10px 20px rgba(37, 99, 235, 0.05), /* Subtle color reflection */
...
```css
box-shadow: 
  16px 16px 32px rgba(148, 163, 184, 0.2), /* Soft slate-blue drop shadow */
  -10px -10px 24px rgba(255, 255, 255, 0.9), /* Strong top-left highlight */
  inset 6px 6px 12px rgba(37, 99, 235, 0.03), /* Inner colored bounce light */
...
```css
box-shadow: 
  12px 12px 24px rgba(37, 99, 235, 0.3), /* Strong blue drop shadow */
...
*   **Base Styles**: `relative rounded-[32px] bg-clay-cardBg p-6 sm:p-8 text-clay-foreground shadow-clayCard backdrop-blur-xl`
*   **Interactive States**:
...
*   **Structure**:
    *   Outer wrapper handles positioning and shadows. **Do not use `overflow-hidden`** as it will clip peeking elements, badges, or thick italicized fonts on the right edge.
...
*   **Variants**:
    *   **Primary/Default**: `bg-gradient-to-br from-[#60A5FA] to-[#2563EB] text-white shadow-clayButton hover:shadow-clayButtonHover`
...
*   **Individual Blobs**:
    ...
    *   Colors: Blue accent colors with `/10` opacity (e.g., `bg-[#2563EB]/10`, `bg-[#0EA5E9]/10`, `bg-[#60A5FA]/10`)
...
- [ ] **Mobile & PC Ready**: Layouts gracefully adapt from narrow mobile screens to wide PC displays.
- [ ] **Background**: Canvas `#F0F6FA` (icy blue) + Animated Blobs.
