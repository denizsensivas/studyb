...
interface SummaryStats {
  ...
  studyMinutes: number;
  topicStudyMinutes: number;
  topicStudyCount: number;
}
...
interface DashboardStats {
  ...
  pomodoroStats: DurationStat[];
  studyStats: DurationStat[];
  examStats: {
...
function SubjectPieChart({ stats = [], label }: { stats?: DurationStat[]; label: string }) {
    ...
    <div className="relative flex flex-col items-center gap-10 lg:flex-row lg:items-center lg:justify-between w-full">
      ...
      <div className="relative h-56 w-56 sm:h-64 sm:w-64 shrink-0 order-1 lg:order-2 flex items-center justify-center">
        ...
        <svg viewBox="0 0 42 42" className="h-full w-full relative top-0 left-0">
          <defs>
            <filter id={filterId} x="-12" y="-12" width="66" height="66" filterUnits="userSpaceOnUse">
          ...
          {segments.map((s) => (
            <g key={s.name}>
              {highlightedIndex === s.index && (
                <path d={s.path} fill="none" stroke={s.color} strokeWidth="7" opacity="0.48" filter={`url(#${filterId})`} pointerEvents="none" aria-hidden="true" />
              )}
              <path
                d={s.path}
                fill="transparent"
                ...
                aria-pressed={selectedIndex === s.index}
                style={{ opacity: highlightedIndex !== null && highlightedIndex !== s.index ? 0.3 : 1 }}
                onClick={() => toggleSelection(s.index)}
                onMouseEnter={() => setHoveredIndex(s.index)}
                onMouseLeave={() => setHoveredIndex(null)}
                onFocus={() => setHoveredIndex(s.index)}
        ...
        <div className="absolute inset-16 sm:inset-20 rounded-full bg-white shadow-clay-card flex flex-col items-center justify-center text-center p-4 z-10 select-none backdrop-blur-sm bg-white/90">
          <span className="text-[10px] font-black text-clay-muted uppercase tracking-widest leading-none mb-1">
            {highlightedIndex !== null ? segments[highlightedIndex].name : label}
          </span>
...
export default function AnalyticsPage() {
  ...
  const maxStudyMinutes = stats?.dailyStats?.length
    ? Math.max(...stats.dailyStats.map((d) => d.topicStudyMinutes || 0), 10)
    : 10;
          ...
          <>
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <Card className="flex flex-col items-center justify-center text-center p-6">
                ...
                <span className="text-3xl font-black text-clay-foreground" style={{ fontFamily: 'Nunito, sans-serif' }}>{averageQuestionTime}s</span>
        ...
        {analyticsTab === 'questions' && (
          <>
            ...
            <Card className="flex flex-col gap-6">
              ...
              <div className="relative h-72 w-full rounded-3xl bg-clay-canvas p-6 pt-12 flex items-end gap-2 shadow-clay-pressed overflow-x-auto">
                {stats?.dailyStats?.every((d) => d.total === 0) ? (
        ...
        {analyticsTab === 'study' && (
          <>
            ...
            <Card className="flex flex-col gap-6">
              ...
              <div className="relative h-72 w-full rounded-3xl bg-clay-canvas p-6 pt-12 flex items-end gap-2 shadow-clay-pressed overflow-x-auto">
                {stats?.dailyStats?.every((d) => (d.topicStudyMinutes || 0) === 0) ? (
              ...
              <Card className="flex flex-col gap-6">
                ...
                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  {stats?.studyStats?.length === 0 ? (
                    ...
                    stats?.studyStats?.map((stat, i) => {
                      const totalMinutes = stat.totalMinutes || 0;
                      const allTotalMinutes = stats?.studyStats?.reduce((a, s) => a + (s.totalMinutes || 0), 0) || 1;
