    ...
    <Layout className="px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl space-y-10">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          <Card className="flex flex-col items-center justify-center p-6 bg-gradient-to-br from-white to-slate-50">
            <StatOrb
              ...
              label={timeRange === 'week' ? "Haftalık Soru" : "Toplam Soru"}
          ...
          <Card className="flex flex-col items-center justify-center p-6 bg-gradient-to-br from-white to-slate-50">
            <StatOrb
              value={`${currentStats.total > 0 ? Math.round((currentStats.correct / currentStats.total) * 100) : 0}%`}
              label="Başarı Oranı"
          ...
          <Card className="flex flex-col items-center justify-center p-6 bg-gradient-to-br from-white to-slate-50">
            <StatOrb
              value={currentStats.pomodoroCount || 0}
              label="Oturum Sayısı"
        ...
        <div className="grid gap-8 lg:grid-cols-3">
                ...
                stats?.dailyStats?.map((day: any) => {
                  ...
                  return (
                    ...
                    <div className="absolute -top-10 left-1/2 -translate-x-1/2 z-20 hidden flex-col items-center rounded-xl bg-slate-900 px-3 py-1.5 text-[10px] font-black text-white opacity-0 transition-opacity group-hover:flex group-hover:opacity-100 whitespace-nowrap shadow-2xl">
                      ...
                      >
                        <div 
                          className="w-full bg-clay-success transition-all duration-500" 
          ...
          {/* Exam Insights Card */}
          <Card className="flex flex-col gap-6 bg-gradient-to-br from-[#EEF2FF] to-white border-none">
            <h2 className="text-2xl font-black text-clay-foreground" style={{ fontFamily: 'Nunito, sans-serif' }}>Sınav Analizi</h2>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-2xl bg-white p-4 shadow-clay-card text-center">
                  <p className="text-[10px] font-black uppercase text-clay-muted leading-tight mb-1">Toplam Seri</p>
                  <p className="text-2xl font-black text-clay-foreground">🔥 {stats?.streak || 0}</p>
                </div>
                <div className="rounded-2xl bg-white p-4 shadow-clay-card text-center">
                  <p className="text-[10px] font-black uppercase text-clay-muted leading-tight mb-1">Süre Ölçülen</p>
                  <p className="text-2xl font-black text-clay-foreground">📝 {stats?.examStats?.totalQuestionsTimed || 0}</p>
                </div>
              </div>
            </div>
          </Card>
        ...
        {/* Pomodoro Study Time Breakdown */}
        <div className="grid gap-8 lg:grid-cols-2">
          {/* Pie Chart Card */}
          <Card className="flex flex-col gap-8 bg-gradient-to-br from-[#F0F9FF] to-white border-none">
            <h2 className="text-2xl font-black text-clay-foreground" style={{ fontFamily: 'Nunito, sans-serif' }}>Konu Dağılımı</h2>
            <div className="flex flex-1 items-center justify-center p-4">
              <SubjectPieChart stats={stats?.pomodoroStats} />
            </div>
          </Card>
          ...
          <Card className="flex flex-col gap-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 px-4 py-1.5 bg-clay-canvas rounded-full shadow-clay-pressed text-xs font-black text-clay-muted uppercase tracking-widest">
                ⏱️ Toplam {Math.floor((currentStats.studyMinutes || 0) / 60)}s {(currentStats.studyMinutes || 0) % 60}d
              </div>
            </div>
                    ...
                    <div key={stat.subjectId || 'none'} className="flex items-center gap-4 bg-clay-canvas/50 p-4 rounded-2xl transition-all hover:bg-white hover:shadow-clay-card">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="h-1.5 w-full bg-white rounded-full overflow-hidden">
                          <div 
                            className="h-full bg-clay-accent transition-all duration-1000"
                            style={{ width: `${percentage}%`, backgroundColor: SUBJECT_COLORS[i % SUBJECT_COLORS.length] }}
                          />
                        </div>
                      </div>
        ...
        {/* Subject Breakdown Card (Exam Success) */}
        <Card className="flex flex-col gap-8">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-black text-clay-foreground" style={{ fontFamily: 'Nunito, sans-serif' }}>Ders Bazlı Başarı</h2>
            <span className="text-xs font-black text-clay-muted bg-clay-canvas px-4 py-1.5 rounded-full uppercase tracking-widest shadow-clay-pressed">Tüm Zamanlar</span>
          </div>

          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {stats?.subjectStats?.length === 0 ? (
              <div className="col-span-full py-12 text-center text-clay-muted font-medium italic bg-clay-canvas/30 rounded-3xl border-2 border-dashed border-slate-200">
              ...
              stats?.subjectStats?.map((subject: any) => (
                <div key={subject.subjectId} className="group relative overflow-hidden rounded-[24px] bg-clay-canvas p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-clay-card hover:bg-white">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className={`text-lg font-black ${subject.accuracy > 75 ? 'text-clay-success' : subject.accuracy > 50 ? 'text-clay-accent' : 'text-clay-warning'}`}>
                      %{subject.accuracy}
                    </span>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="h-2.5 w-full bg-white rounded-full overflow-hidden shadow-clay-pressed">
                      <div 
                        className={`h-full transition-all duration-1000 ${subject.accuracy > 75 ? 'bg-clay-success' : subject.accuracy > 50 ? 'bg-clay-accent' : 'bg-clay-warning'}`}
                    ...
                    <div className="flex items-center justify-between text-xs font-bold text-clay-muted">
                      <div className="flex items-center gap-1.5">
                        <span className="h-1.5 w-1.5 rounded-full bg-clay-success"></span>
                        <span>{subject.correct} Doğru</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className="h-1.5 w-1.5 rounded-full bg-clay-warning"></span>
